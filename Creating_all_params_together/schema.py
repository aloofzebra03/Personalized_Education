...
class StudentParameters(BaseModel):
    ...
    difficulty: int = Field(
        ...,
        ge=1, le=5,
        description="According to current student parameters, the difficulty level (1–5) appropriate for the selected next section"
    )
    board_exam_importance: int = Field(
        ...,
        ge=1, le=5,
        description="According to current student parameters, how important this next section is for board exams (1–5)"
    )
    olympiad_importance: int = Field(
        ...,
        ge=1, le=5,
        description="According to current student parameters, how important this next section is for Olympiad preparation (1–5)"
    )
    avg_study_time_min: int = Field(
        ...,
        ge=1, le=5,
        description="According to current student parameters, the estimated study time (1–5 scale) for the selected next section"
    )
    interest_evoking: int = Field(
        ...,
        ge=1, le=5,
        description="According to current student parameters, how engaging this next section will be (1–5)"
    )
    curiosity_evoking: int = Field(
        ...,
        ge=1, le=5,
        description="According to current student parameters, how much this next section will spark curiosity (1–5)"
    )
    critical_reasoning_needed: int = Field(
        ...,
        ge=1, le=5,
        description="According to current student parameters, the level of critical reasoning required for this next section (1–5)"
    )
    inquiry_learning_scope: int = Field(
        ...,
        ge=1, le=5,
        description="According to current student parameters, scope for inquiry-based learning in this next section (1–5)"
    )
    example_availability: int = Field(
        ...,
        ge=1, le=5,
        description="According to current student parameters, the availability of helpful examples in this next section (1–5)"
    )
