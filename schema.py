...
class StudentProfile(BaseModel):
    ...
    class_level: int = Field(gt=6, lt=12, description="Current class level of the student, between 7 and 11")
    ...
    learning_style: str = Field(description="Dominant learning style like Visual, Auditory, or Kinesthetic.There can be more.Ensure that each element in the List is only a single word and not a sentence")
    pattern_of_learning: List[str] = Field(description="Preferred methods of learning such as reading, writing, or hands-on activitiesThere can be more.Ensure that each element in the List is only a single word and not a sentence")
    
    study_routine_start: time = Field(description="Start time of study routine")
    study_routine_end: time = Field(description="End time of study routine")
    ...
    struggles_with: str = Field(description="Subject or areas of a particular subject the student finds most challenging")
    ...
    motivation_style: List[str] = Field(description="What motivates the student—rewards, praise, recognition, etc.Ensure that each element in the List is only a single word and not a sentence")
    ...
    tech_savviness: Literal['High','Medium','Low'] = Field(description="Level of comfort and skills with technology")
