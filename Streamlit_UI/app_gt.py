...
from Creating_Section_Text.pipeline import ingest_and_build_store, run_one_gt
# ──────────────────────────────────────────────────────────────────────────────
...
if st.session_state.choice and st.session_state.content is None:
    with st.spinner(f"Generating “{st.session_state.choice.section_name}”…"):
        st.session_state.content = run_one_gt(
            student_params["ongoing_concept"],
            st.session_state.choice
        )

# 9) Display the next section & content
st.markdown(f"### Ground Truth found for Next Section: **{st.session_state.choice.section_name}**")
